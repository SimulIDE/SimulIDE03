...
Simulator::~Simulator() { m_CircuitFuture.waitForFinished(); }

inline void Simulator::solveMatrix() {
  foreach (eNode *node, m_eChangedNodeList)
    node->stampMatrix();
  m_eChangedNodeList.clear();

  if (!m_matrix.solveMatrix()) // Try to solve matrix,
  {                            // if fail stop simulation
    std::cout << "Simulator::solveMatrix(), Failed to solve Matrix"
              << std::endl;
    m_error = true;
  } // m_matrix sets the eNode voltages
}

void Simulator::timerEvent(
    QTimerEvent *e) // update at m_timerTick rate (50 ms, 20 Hz max)
{
  e->accept();
...
void Simulator::runGraphicStep() {
  ...
  foreach (eElement *el, m_updateList)
    el->updateStep();
  ...
  foreach (eNode *enode, m_eNodeList)
...
void Simulator::runCircuitStep() {
  ...
  foreach (eElement *el, m_simuClock)
    el->simuClockStep();
  ...
  if (BaseProcessor::self() && !m_debugging)
  ...
  if (++m_noLinCounter >= m_stepsNolin) {
    ...
    while (!m_nonLinear.isEmpty()) // Run untill all converged
    {
      ...
      if (++counter > limit)
        break; // Limit the number of loops
    }
    // if( counter > 0 ) qDebug() << "\nSimulator::runCircuit  Non-Linear Solved
    // in steps:"<<counter;
  }
  if (!m_eChangedNodeList.isEmpty()) {
    solveMatrix();
...
void Simulator::startSim() {
  foreach (eNode *busNode, m_eNodeBusList)
    busNode->initialize();              // Clear Buses
  ...
  std::cout << "\nInitializing " << m_eNodeBusList.size() << " Buses"
            << std::endl;
  foreach (eNode *busNode, m_eNodeBusList)
    busNode->createBus(); // Create Buses
  foreach (eElement *el, m_elementList)
    el->initialize(); // Initialize all Elements
  ...
  if (!m_matrix.solveMatrix()) {
    std::cout << "Simulator::startSim, Failed to solve Matrix" << std::endl;
    CircuitWidget::self()->powerCircOff();
    CircuitWidget::self()->setRate(-1);
    return;
  }
...
void Simulator::stopDebug() { m_debugging = false; }
...
void Simulator::stopSim() {
  ...
  foreach (eNode *node, m_eNodeList)
    node->setVolt(0);
  foreach (eElement *el, m_elementList) {
    el->initialize();
    el->resetState();
...
int Simulator::simuRateChanged(int rate) {
  ...
  std::cout << "\nFPS:              " << fps
            << "\nCircuit Rate:     " << m_circuitRate << std::endl
            << "\nSimulation Speed: "
            << m_simuRate
            /*<< "\nReactive   Speed: " << m_simuRate/m_stepsPrea*/
            << "\nReactive SubRate: " << m_stepsPrea
            << "\nNoLinear Subrate: " << m_stepsNolin << std::endl
            << std::endl;
...
bool Simulator::isPaused() { return m_paused; }

int Simulator::reaClock() { return m_stepsPrea; }
...
int Simulator::noLinClock() { return m_stepsNolin; }
...
uint64_t Simulator::step() { return m_step; }

void Simulator::addToEnodeBusList(eNode *nod) {
  if (!m_eNodeBusList.contains(nod))
    m_eNodeBusList.append(nod);
}

void Simulator::remFromEnodeBusList(eNode *nod) {
  if (m_eNodeBusList.contains(nod))
    m_eNodeBusList.removeOne(nod);
}
...
void Simulator::addToChangedNodeList(eNode *nod) {
  if (!m_eChangedNodeList.contains(nod))
    m_eChangedNodeList.append(nod);
}
...
void Simulator::addToUpdateList(eElement *el) {
  if (!m_updateList.contains(el))
    m_updateList.append(el);
}

void Simulator::remFromUpdateList(eElement *el) { m_updateList.removeOne(el); }

void Simulator::addToSimuClockList(eElement *el) {
  if (!m_simuClock.contains(el))
    m_simuClock.append(el);
}
...
void Simulator::addToNoLinList(eElement *el) {
  if (!m_nonLinear.contains(el))
    m_nonLinear.append(el);
}

void Simulator::remFromNoLinList(eElement *el) { m_nonLinear.removeOne(el); }

void Simulator::addToMcuList(BaseProcessor *proc) {
  if (!m_mcuList.contains(proc))
    m_mcuList.append(proc);
}
void Simulator::remFromMcuList(BaseProcessor *proc) {
  m_mcuList.removeOne(proc);
}
